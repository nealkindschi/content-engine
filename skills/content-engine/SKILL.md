...
- **Corporate product and demo links**: Never link to corporate product pages, demo/signup pages, pricing pages, or "request a demo" CTAs in the final article -- even if those pages were used for fact verification in Stage 6. Verification sources may be corporate sources (e.g. company research portals, security documentation), but the final published article must not point readers to a specific vendor's sales funnel. If a verification source is a corporate product page, cite it in the fact-check report but do NOT include it in the article.
